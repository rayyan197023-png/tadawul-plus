'use client';
/**
 * useDividend -- جلب توزيعات سهم واحد عند الطلب
 * useDividendsList -- جلب قائمة أسهم (دفعات)
 */
...
export function useDividend(sym) {
  ...
  useEffect(() => {
    ...
    if (cache[sym]) { setData(cache[sym]); return; }
    ...
    fetch(`/api/sahmkdata?endpoint=dividends&sym=${sym}`)
      .then(r => { if (!r.ok) throw new Error('فشل الجلب'); return r.json(); })
      .then(json => {
        if (cancelled) return;
        if (json && json.error) { setError(json.error); setData(null); }
        else { cache[sym] = json; setData(json); }
      })
...
// ── Hook جماعي: قائمة أسهم (دفعات) ──
export function useDividendsList(stocksList) {
  const [items, setItems]   = useState([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    if (!stocksList || stocksList.length === 0) { setLoading(false); return; }
