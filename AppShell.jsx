      ...
      {isStockOpen && liveStock && (
        <div style={{ position:'fixed', inset:0, zIndex:200, background:C.bg }}>
          <ErrorBoundary label="صفحة السهم">
            <Suspense fallback={<Loader />}>
              <StockDetail stk={liveStock} onClose={() => { haptic.tap(); closeStock(); }} wl={wlSyms} toggleStar={(sym) => {
                setWatchlist(prev => prev.some(w=>w.sym===sym) ? prev.filter(w=>w.sym!==sym) : [...prev,{sym,name:sym,color:'#f0c050'}]);
              }} />
            </Suspense>
          </ErrorBoundary>
        </div>
