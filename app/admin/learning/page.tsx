...
const prompt = `أنت نظام تحسين ذاتي لمحلل AI متخصص في السوق السعودي == بيانات الأداء == إجمالي التحليلات: ${records.length} المقيَّمة: ${evaluated.length} الدقة الكلية: ${acc !== null ? acc + "%" : "غير محددة"} صحيحة: ${evaluated.filter(r=>r.outcome==="correct").length} | خاطئة: ${evaluated.filter(r=>r.outcome==="wrong").length} == أنماط الأخطاء == ${errorList.length ? errorList.join("\n") : "لا توجد بيانات كافية"} == آخر 3 تحليلات مقيَّمة == ${evaluated.slice(-3).map(r => `${r.name}(${r.sym}): ${r.outcome} | أخطاء: ${r.errorPatterns?.join(”, “)||“لا”}`).join(”\n”)}
اكتب تحليلاً ذاتياً دقيقاً يشمل:

1. **تشخيص الأنماط** — ما المتكرر ولماذا؟
1. **جذر المشكلة** — السبب الحقيقي
1. **تحسينات الـ Prompts** — 3 تعليمات إضافية محددة
1. **قواعد التحقق** — 5 قواعد صارمة قبل أي توصية
...
<style>{`@import url('https://fonts.googleapis.com/css2?family=Cairo:wght@400;600;700;800;900&display=swap'); @keyframes spin { to { transform:rotate(360deg); } } @keyframes pulse { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.4;transform:scale(.95)} } @keyframes fadeIn { from{opacity:0;transform:translateY(5px)} to{opacity:1;transform:translateY(0)} } @keyframes shimmer { 0%{background-position:200% 0} 100%{background-position:-200% 0} } * { box-sizing:border-box; margin:0; padding:0; } button,input,textarea { font-family:inherit; } ::-webkit-scrollbar { width:3px; } ::-webkit-scrollbar-thumb { background:${C.edge}; border-radius:3px; } input::placeholder,textarea::placeholder { color:${C.ash}; }`}</style>
