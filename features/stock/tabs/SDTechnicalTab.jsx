...
function ElliottWaveAI({ stk, hist }) {
  ...
  const analyzeWaves = (pivots, bars, timeframe) => {
    ...
    const WAVE_PROPS = {
      ...
      "C": {
        title: "موجة C -- إتمام التصحيح",
        chars: "أقوى وأعنف من موجة A وتتجاوزها دائماً. حجم تداول مرتفع. تتكون من 5 موجات فرعية. من دخل في موجة B يتكبد خسائر.",
        rules: "تساوي A عادة (100%) أو 161.8% منه. نهايتها = بداية دورة جديدة في الاتجاه المعاكس.",
        signal: "اكتمال C = نقطة انعكاس مرشّحة.",
      },
      "C_up": {
        title: "موجة C صاعدة -- ارتداد تصحيحي",
        chars: "ارتداد صاعد ضمن اتجاه هابط أكبر. يتجاوز قمة A. يوهم بانتهاء الهبوط لكنه تصحيح لا انعكاس.",
        rules: "تساوي A عادة (100%) أو 161.8% منه. نهايتها = استئناف الهبوط الرئيسي.",
        signal: "⚠️ ارتداد مؤقت -- ليس بداية صعود جديد.",
      },
    };
    ...
    if (imp.valid) {
      ...
      if (beyond5) {
        ...
        // ✨ الهدف = المستوى التالي فوق ما تحقق فعلاً
        var _lv = [1.0, 1.272, 1.618, 2.0, 2.618];
