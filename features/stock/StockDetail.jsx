...
import { fetchFundamentals, fetchRealTimeQuote } from '../../services/api/eodhdApi';
...
function ScoreDrawer({ item, color, onClose }) {
  ...
  return (
    <>
      <div onClick={onClose}
        style={{ position:"fixed", inset:0, zIndex:9998, background:"rgba(0,0,0,.65)" }}/>
  ...
  return (
    <div>
      ...
      {open && rect && (
        <>
          <div onClick={()=>setOpen(false)} style={{ position:"fixed", inset:0, zIndex:998 }}/>
          <div style={{ position:"fixed", top:rect.top - 212, left: Math.max(8, rect.right - 148), background:C.layer1, border:`1px solid ${C.line}`, borderRadius:10, padding:6, boxShadow:"0 8px 24px rgba(0,0,0,.8)", zIndex:999, display:"grid", gridTemplateColumns:"1fr 1fr", gap:3, minWidth:140 }}>
...
const dayPos = Math.min(100, Math.max(0, ((stk.p-dayLo)/dayRng)*100));
  ...
  const statRowsMain = [
    ...
    (stk.bvps||stk.bookValue)?{l:"القيمة الدفترية/السهم",v:(stk.bvps||stk.bookValue)+" ر.س"}:null,
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
      ...
      {/* نطاقات الأسعار */}
      {[
        {title:"النطاق اليومي", lo:(stk.dayLo??stk.lo??'-')+' ر.س', hi:(stk.dayHi??stk.hi??'-')+' ر.س', pos:dayPos},
...
// ─── SDTechnical ─────────────────────────────────────────────────
// ─── ElliottWaveCard — تحليل موجات إيليوت AI ───
function ElliottWaveCard({ stk, hist }) {
  ...
  const analyzeEW = async () => {
    ...
    try {
      ...
      const prompt = `حلّل سهم ${stk.name} (${stk.sym}) بموجات إيليوت. السعر: ${stk.p} ر.س. شهري: ${monthly}. أسبوعي: ${weekly}. يومي: ${daily}. JSON: {"primary":{"wave":"3","dir":"صاعد","type":"ممتدة","note":""},"intermediate":{"wave":"iii","dir":"صاعد","type":"ممتدة","note":""},"minor":{"wave":"3","dir":"صاعد","type":"طبيعية","note":""},"currentPos":"","nextTarget":{"bull":0,"bear":0,"fib":""},"support":0,"resistance":0,"warning":"","summary":""}`;
      ...
      <div style={{ padding:"10px 16px" }}>
        {!ewData && !ewLoading && (
          <div style={{ textAlign:"center", padding:"16px 0" }}>
            <div style={{ fontSize:11, color:C.smoke, marginBottom:12, lineHeight:1.6 }}>
              تحليل AI احترافي للموجات على الإطارات الشهري والأسبوعي واليومي
            </div>
            <button onClick={analyzeEW}
              style={{ display:"inline-flex", alignItems:"center", gap:8, background:`linear-gradient(135deg,${C.plasma}33,${C.electric}22)`, border:`1px solid ${C.plasma}66`, borderRadius:12, padding:"12px 24px", color:C.plasma, fontSize:12, fontWeight:800, cursor:"pointer", fontFamily:"Cairo,sans-serif" }}>
        ...
        {ewLoading && (
          <div style={{ textAlign:"center", padding:"20px 0" }}>
            <div style={{ fontSize:11, color:C.smoke }}>يحلل AI الموجات...</div>
          </div>
        )}
        {ewErr && (
        ...
        {ewData && (
          <>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:6, marginBottom:10 }}>
              {[
                {label:"شهري",   data:ewData.primary},
                {label:"أسبوعي", data:ewData.intermediate},
                {label:"يومي",   data:ewData.minor},
              ].map((tf,i)=>{
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", marginTop:8 }}>
              <button onClick={analyzeEW} disabled={ewLoading}
                style={{ background:C.plasma+"15", border:`1px solid ${C.plasma}44`, borderRadius:7, padding:"5px 10px", color:C.plasma, fontSize:10, fontWeight:700, cursor:"pointer", fontFamily:"Cairo,sans-serif" }}>
                تحديث
              </button>
            </div>
          </>
...
<text x={CX+R+4} y={CY+14} textAnchor="middle" fill={C.mint} fontSize="8">شراء</text>
      ...
      {/* Elliott Wave — تحليل AI - مُحوَّل لكومبوننت */}
      <ElliottWaveCard stk={stk} hist={hist} />
...
// ─── SDFundamental ───────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════
// كومبوننتات تبويب "الأساسي" - مُستخرَجة من IIFE معطوبة
// كانت تستخدم useState داخل IIFE في JSX مما يخرق قواعد React Hooks
// ═══════════════════════════════════════════════════════════════════

// ─── DCFValuationCard — تقييم السهم بطريقة DCF ───
function DCFValuationCard({ stk }) {
  const [dcfG, setDcfG]  = useState(stk.revGrowthYoY || stk.epsGrw || 8);
  ...
  const dcfVal = _dcfRaw || stk.fv || stk.target;
  const dcfUpside = dcfVal ? parseFloat(((dcfVal - stk.p) / stk.p * 100).toFixed(1)) : 0;
  const dcfC = stk.p < (dcfVal || stk.p) ? C.mint : C.coral;
  const displayUpside = dcfVal ? dcfUpside.toFixed(1) : "0";
  ...
  return (
    <SectionCard title="تقييم السهم" accent={dcfC}>
      ...
      {!dcfVal
        ? <EmptyState icon="🔍" title="القيمة العادلة غير متوفرة" subtitle="بيانات السهم لا تكفي لاحتساب القيمة العادلة"/>
        : <div style={{ padding:"14px 16px" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
            ...
            <div style={{ textAlign:"center" }}>
              <div style={{ background:dcfC+"18", border:`1px solid ${dcfC}44`, color:dcfC, fontSize:11, fontWeight:800, padding:"6px 14px", borderRadius:10 }}>{stk.p<dcfVal?"مقيّم بأقل":"مبالغ في التقييم"}</div>
              <div style={{ fontFamily:"IBM Plex Mono,monospace", fontSize:13, fontWeight:900, color:dcfC, marginTop:6, textShadow:`0 0 8px ${dcfC}55`, lineHeight:1.5 }}>{stk.p<dcfVal?"+":""}{displayUpside}%</div>
              ...
              <button onClick={()=>setShowDCF(v=>!v)} style={{ marginTop:6, fontSize:10, color:C.electric, background:"transparent", border:`1px solid ${C.electric}44`, borderRadius:6, padding:"3px 10px", cursor:"pointer", fontFamily:"Cairo,sans-serif" }}>{showDCF?"إخفاء المحاكي":"عرض محاكي DCF"}</button>
            </div>
            <div style={{ textAlign:"left" }}>
              <div style={{ fontFamily:"IBM Plex Mono,monospace", fontSize:22, fontWeight:900, color:C.mint, letterSpacing:"-1px", textShadow:`0 0 10px ${C.mint}55`, lineHeight:1 }}>{dcfVal} <span style={{ fontSize:11, color:C.smoke, fontWeight:400 }}>ر.س</span></div>
          ...
          <div style={{ height:6, background:C.layer3, borderRadius:3, overflow:"hidden", position:"relative" }}>
            <div style={{ position:"absolute", right:0, top:0, height:"100%", width:`${Math.min((stk.p/Math.max(stk.p,dcfVal))*100,100)}%`, background:`linear-gradient(270deg,${dcfC},${dcfC}88)`, borderRadius:3 }}/>
            <div style={{ position:"absolute", top:-2, right:`${Math.min((dcfVal/Math.max(stk.p,dcfVal))*100,98)}%`, width:2, height:10, background:C.gold, borderRadius:1 }}/>
          </div>
        </div>}
    </SectionCard>
  );
...
// ─── ValuationMultiplesCard — مضاعفات التقييم مع حسابات ذكية ───
function ValuationMultiplesCard({ stk }) {
  const [valInfo, setValInfo] = useState(null);
  const valCol = (v, good, high) => !v ? C.smoke : v<=good ? C.mint : v<=high ? C.amber : C.coral;

  const computedPEG = (stk.pegRatio != null) ? stk.pegRatio
                    : (stk.pe && stk.epsGrw && stk.epsGrw > 0) ? parseFloat((stk.pe / stk.epsGrw).toFixed(2))
                    : null;
  const computedPB  = (stk.pb != null) ? stk.pb
                    : (stk.p && stk.bookValue && stk.bookValue > 0) ? parseFloat((stk.p / stk.bookValue).toFixed(2))
                    : null;
  const computedFwdPE = (stk.forwardPE != null) ? stk.forwardPE
                      : (stk.epsForward && stk.epsForward > 0) ? parseFloat((stk.p / stk.epsForward).toFixed(2))
                      : (stk.pe && stk.epsGrw) ? parseFloat((stk.pe / (1 + stk.epsGrw/100)).toFixed(2))
                      : null;
  const computedPCF = (stk.pcf != null) ? stk.pcf
                    : (stk.p && stk.freeCashFlow && stk.freeCashFlow > 0) ? parseFloat((stk.p / stk.freeCashFlow).toFixed(2))
                    : null;
  const computedPS = stk.ps != null ? stk.ps : null;

  const items = [
    {l:"مضاعف الأرباح الحالي",  v:stk.pe,        sec:stk.sectorPE, col:valCol(stk.pe,15,25),
      tip:"السعر / الربح السنوي. جيد <15x"},
    {l:"مضاعف الأرباح المتوقع", v:computedFwdPE, sec:null,         col:valCol(computedFwdPE,14,20),
      tip:"السعر / الأرباح المتوقعة"},
    {l:"نسبة PEG",              v:computedPEG,   sec:null,         col:valCol(computedPEG,1,2),
      tip:"P/E / معدل نمو الأرباح. جذاب <1"},
    {l:"السعر/القيمة الدفترية", v:computedPB,    sec:stk.sectorPB, col:valCol(computedPB,1.5,3),
      tip:"السعر / القيمة الدفترية"},
    {l:"السعر/المبيعات",        v:computedPS,    sec:null,         col:valCol(computedPS,2,5),
      tip:"القيمة السوقية / الإيرادات"},
    {l:"السعر/التدفق الحر",      v:computedPCF,   sec:null,         col:valCol(computedPCF,15,25),
      tip:"السعر / التدفق النقدي الحر"},
    {l:"EV/EBITDA",             v:stk.evebitda,  sec:null,         col:valCol(stk.evebitda,8,12),
      tip:"قيمة المنشأة / EBITDA"},
    {l:"EV/EBIT",               v:stk.evEbit,    sec:null,         col:valCol(stk.evEbit,10,15),
      tip:"قيمة المنشأة / الربح التشغيلي"},
    {l:"قيمة المنشأة/المبيعات", v:stk.evSales,   sec:null,         col:valCol(stk.evSales,3,6),
      tip:"قيمة المنشأة / الإيرادات"},
  ];

  return (
    <SectionCard title="مضاعفات التقييم" accent={C.gold}>
      {valInfo && (
...
// ─── ProfitMarginsCard — هوامش الربحية ───
function ProfitMarginsCard({ stk }) {
  ...
  const hasAny = items.some(it => it.v != null);
  return (
    <SectionCard title="هوامش الربحية" accent={C.mint} infoBtn={{onClick:()=>setShowInfo(v=>!v)}}>
      ...
      <div style={{ padding:"12px 16px" }}>
        {!hasAny ? (
          <div style={{ fontSize:11, color:C.smoke, textAlign:"center", padding:"8px 0" }}>بيانات هوامش الربحية غير متوفرة</div>
        ) : items.map((item,i) => item.v ? (
          <div key={i} style={{ marginBottom:10 }}>
            <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
              <div><span style={{ fontSize:11, color:C.mist, lineHeight:1.5 }}>{item.l}</span></div>
...
// ─── ReturnsEfficiencyCard — العوائد والكفاءة ───
function ReturnsEfficiencyCard({ stk }) {
  const [showInfo, setShowInfo] = useState(false);
  const items = [
  ...
  return (
    <SectionCard title="العوائد والكفاءة" accent={C.teal} infoBtn={{onClick:()=>setShowInfo(v=>!v)}}>
      ...
      <div style={{ padding:"0" }}>
        {items.length === 0 ? (
          <div style={{ fontSize:11, color:C.smoke, textAlign:"center", padding:"12px" }}>بيانات العوائد غير متوفرة</div>
        ) : items.map((item,i) => (
...
// ─── ProsConsCard — إيجابيات وسلبيات ───
function ProsConsCard({ stk, pc }) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:800,
          tools:[{type:"web_search_20250305",name:"web_search"}],
          messages:[{role:"user",content:'ابحث عن أبرز 4 نقاط قوة و4 مخاطر لسهم '+stk.name+' ('+stk.sym+'). أجب بـ JSON: {"pros":["..."],"cons":["..."]}'}]})
      });
      const d = await res.json();
      const txt = (d.content||[]).filter(b=>b.type==="text").map(b=>b.text).join("");
      const m = txt.match(/\{[\s\S]*\}/);
...
// ─── DisclosuresCard — الإفصاحات والأخبار ───
function DisclosuresCard({ stk, disc }) {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1200,
          tools:[{type:"web_search_20250305",name:"web_search"}],
          messages:[{role:"user",content:'ابحث عن أحدث 5 إفصاحات لسهم '+stk.name+' ('+stk.sym+'). JSON: {"items":[{"title":"...","date":"YYYY/MM/DD","type":"...","impact":"..."}]}'}]})
      });
      const d = await res.json();
      const txt = (d.content||[]).filter(b=>b.type==="text").map(b=>b.text).join("");
      const m = txt.match(/\{[\s\S]*\}/);
  ...
  return (
    <SectionCard title="الإفصاحات والأخبار الأخيرة" accent={C.teal}
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 16px 0" }}>
        {discFetched
          ? <span style={{ fontSize:9, color:C.mint }}>{"✓ بيانات حية"}</span>
          : <span style={{ fontSize:9, color:C.smoke }}>اضغط للتحديث</span>}
        <button onClick={fetchDisc} disabled={discLoading}
          style={{ background:discLoading?C.layer3:`${C.teal}15`, border:`1px solid ${C.teal}44`, borderRadius:7, padding:"5px 10px", color:C.teal, fontSize:10, fontWeight:700, cursor:discLoading?"not-allowed":"pointer", fontFamily:"Cairo,sans-serif" }}>
          {discLoading?"جارٍ...":"تحديث حي"}
        </button>
      </div>
      {discErr && <div style={{ margin:"4px 16px", padding:"5px 10px", background:C.coral+"15", borderRadius:7, fontSize:10, color:C.coral }}>{discErr}</div>}
      {discData.map((d,i) => {
        const tC = d.type==="نتائج مالية"?C.electric:d.type==="توزيعات"?C.gold:C.teal;
        const impC = d.impact==="عالي"?C.coral:d.impact==="متوسط"?C.amber:C.smoke;
        return (
          <div key={i} style={{ padding:"10px 16px", borderBottom:i<discData.length-1?`1px solid ${C.line}22`:0 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8, marginBottom:3 }}>
              <span style={{ fontSize:11, color:C.mist, fontWeight:600, flex:1, lineHeight:1.5 }}>{d.title}</span>
              <div style={{ display:"flex", gap:4, flexShrink:0 }}>
                <Tag text={d.impact} color={impC}/>
...
function SDFundamental({ stk }) {
  ...
  // ملاحظة: حُذفت fvC و upside لأن stk.fv غير موجود في معظم البيانات،
  // والـ DCFValuationCard الجديد يحسب القيمة العادلة وهامش الأمان داخلياً
  ...
  const OverviewPane = () => (
    <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
      ...
      {/* DCF + تقييم السهم — مُحوَّل لكومبوننت */}
      <DCFValuationCard stk={stk} />

      {/* مضاعفات التقييم — مُحوَّل لكومبوننت */}
      <ValuationMultiplesCard stk={stk} />

      {/* هوامش الربحية - مُحوَّل لكومبوننت */}
      <ProfitMarginsCard stk={stk} />

      {/* العوائد - مُحوَّل لكومبوننت */}
      <ReturnsEfficiencyCard stk={stk} />

      {/* إيجابيات وسلبيات - مُحوَّل لكومبوننت */}
      <ProsConsCard stk={stk} pc={pc} />

      {/* الإفصاحات - مُحوَّل لكومبوننت */}
      <DisclosuresCard stk={stk} disc={disc} />
  ...
  const DupontPane = () => {
    ...
    // Graham Number: bvps إذا موجود، وإلا bookValue (الأشهر في البيانات السعودية)
    const _bvpsFinal = stk.bvps || stk.bookValue;
    const graham= stk.eps&&_bvpsFinal ? parseFloat(Math.sqrt(22.5*stk.eps*_bvpsFinal).toFixed(2)) : null;
            ...
            <div style={{ padding:"12px 16px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                <div>
                  ...
                  <div style={{ fontSize:10, color:C.smoke }}>{"= √(22.5 × "}{stk.eps}{" × "}{_bvpsFinal}{")"}</div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <div style={{ textAlign:"center" }}>
...
// ─── TickFilterPanel — فلاتر سجل الصفقات ───
function TickFilterPanel({ ticks, fmtQty, fmtVal }) {
  ...
  return (
    <SectionCard title="سجل الصفقات والتدفق" accent={C.plasma} badge={{text:"يتحدث كل 8 ث ●",color:C.mint}}>
      <div style={{ padding:"8px 16px" }}>
        ...
        {/* فلاتر سجل الصفقات - مُحوَّل لكومبوننت */}
        <TickFilterPanel ticks={ticks} fmtQty={fmtQty} fmtVal={fmtVal} />
...
// ─── PriceAlertButton — زر التنبيه السعري ───
function PriceAlertButton({ stk }) {
    ...
    <div>
      <button ref={abRef}
        ...
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={C.gold} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
          ...
          {(triggered2||alertSet2) && <circle cx="18" cy="8" r="3" fill={triggered2?C.coral:C.gold} stroke="none"/>}
        </svg>
      </button>
      {alertOpen2 && abRect && (
        <>
          <div onClick={()=>setAlertOpen2(false)} style={{ position:"fixed", inset:0, zIndex:998 }}/>
          <div style={{ position:"fixed", top:abRect.bottom+6, left:Math.max(8, Math.min(abRect.left, window.innerWidth-220)), background:C.layer1, border:`1px solid ${C.line}`, borderRadius:12, padding:14, boxShadow:"0 8px 28px rgba(0,0,0,.8)", zIndex:999, minWidth:210 }}>
...
const SAHMK_KEY    = "shmk_live_73567b1dd01323431bc63b568bdafbc8508f27f17e2633cd";
const SAHMK_BASE   = "https://app.sahmk.sa/api/v1";
const LOCAL_SERVER = "http://localhost:5001"; // sahmk_server.py
...
const fetchSahmkQuote = async (sym) => {
  try {
    ...
    console.warn("SAHMK quote:", e.message);
...
const fetchSahmkCompany = async (sym) => {
  try {
    ...
    console.warn("SAHMK company:", e.message);
    return null;
  }
};
        ...
        <div style={{ padding:"4px 20px 8px", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
            ...
            <div style={{ display:"flex", gap:8, alignItems:"center" }}>
              ...
              {/* زر التنبيه السعري - مُحوَّل لكومبوننت */}
              <PriceAlertButton stk={stk} />
