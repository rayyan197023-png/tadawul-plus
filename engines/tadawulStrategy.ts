...
export var STRATEGY_DEFAULTS = {
  ...
  stopLossPct: -0.10,          // المعايرة المثبتة بالاختبار
  takeProfitPct: 0.20,         // المعايرة المثبتة بالاختبار
...
export function createTadawulStrategy(healthFn: any, options?: any, macroOverride?: any, weightsOverride?: any): any { 
  ...
  return {
    ...
    generateSignals: function(day: any, state: any, historicalData: any[], dayIndex: number): any[] {
      var signals: any[] = [];
      if (dayIndex === 30) alert('sellThr=' + config.sellScoreThreshold + ' stopLoss=' + config.stopLossPct + ' maxHold=' + config.maxHoldDays);
      // ① تحديد Market Regime
