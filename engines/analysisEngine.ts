...
function calc9Layers(stk: any, bars: any[]): any {
  ...
  // ════════════════════════════════════════
  // ════════════════════════════════════════
  //  ✨ prior الجديد -- مقياس اتساق الطبقات (بدل المرآة)
  //  المنطق: بدل قراءة قيم L1/L4/L5 (تكرار)، نقرأ مدى اتفاق
  //  ثلاث طبقات متباعدة الأبعاد: L1(هيكل) L8(أساسيات) L9(سيولة).
  //  إجماع عالٍ (تشتّت منخفض) → prior مرتفع | تضارب → منخفض.
  //  هذه معلومة جديدة لا توجد في أي طبقة منفردة.
  // ════════════════════════════════════════
  // القديم (مرآة -- معلّق للتراجع السريع):
  // const priorRaw = (L1*0.60 + L4*0.25 + L5*0.15) / 100;
  const _consMean = (L1 + L8 + L9) / 3;
  const _consStd  = Math.sqrt(
    ((L1-_consMean)**2 + (L8-_consMean)**2 + (L9-_consMean)**2) / 3
  );
  // اتساق: تشتّت 0 → 1.0 | تشتّت 40+ → 0.0
  const _consistency = Math.max(0, 1 - _consStd/40);
  // الاتجاه: هل الإجماع صعودي أم هبوطي؟ (المتوسط فوق/تحت 50)
  const _consDir = _consMean >= 50 ? 1 : 0.55;
  // prior = اتساق موجّه: إجماع صعودي قوي → عالٍ، تضارب → ~0.5
