...
function buildDynamicWeights(regime: string, sector: string): any {
  ...
  // ✨ إعادة توزيع وزن L8 (التقييم الجوهري) -- معطّل في الباك-تيست لغياب
  //    الأساسيات من Yahoo، فوزنه 15% كان يذهب لدرجة محايدة ثابتة تُخفّف الإشارة
  const BASE: any = {
...
function calcFactorLayer11(stk: any, bars: any[]): any {
  ...
  // ① الزخم 12-1 (يتطلّب 252 يوماً)
  let momScore = 50;
  if (n >= 252) {
    const p_now = bars[n - 22].c;              // نتخطّى آخر 21 يوماً (شهر)
  ...
  // ② الجودة -- roe + هامش الربح − الدَّين
  let qualScore = 50;
  let qualAvail = false;
  if (stk.roe != null) {
    qualAvail = true;
    // roe ربعي في sahmk -- نُسنويه تقريبياً ×4
  ...
  // ③ تدنّي التقلّب (BAB) -- التذبذب المنخفض يتفوّق معدّلاً بالمخاطر
  let volScore = 50;
  if (n >= 60) {
    const rets: number[] = [];
  ...
  // الدمج -- نُعيد التوزيع عند غياب الجودة
  const L11 = qualAvail
...
export function stockHealth(stk: any, bars: any[], allStocks?: any[], macroOverride?: any, weightsOverride?: any): any {
  ...
  // LA وحده -- دمج LB/LC يخفض الدرجة تحت العتبة فتقلّ الصفقات للنصف
