...
// Ichimoku
const calcIchi=(h,l,c2)=>{const n=h.length;const mid=(per,i,_h,_l)=>{if(i<per-1)return null;return(Math.max(..._h.slice(i-per+1,i+1))+Math.min(..._l.slice(i-per+1,i+1)))/2;};const tenkan=h.map((_,i)=>mid(9,i,h,l));const kijun=h.map((_,i)=>mid(26,i,h,l));const N26=n+26;const senkouA=new Array(N26).fill(null);tenkan.forEach((t,i)=>{if(t==null||kijun[i]==null)return;const fi=i+26;if(fi<N26)senkouA[fi]=(t+kijun[i])/2;});const senkouB=new Array(N26).fill(null);h.forEach((_,i)=>{const v=mid(52,i,h,l);if(v==null)return;const fi=i+26;if(fi<N26)senkouB[fi]=v;});const chikou=c2?c2.slice():null;return{tenkan,kijun,senkouA,senkouB,chikou};};
