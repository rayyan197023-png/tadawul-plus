...
const vwap=(h,l,c2,v,candles)=>{let ct=0,cv=0,ct2=0,prevD=null;return c2.map((_,i)=>{const t=candles&&candles[i]?new Date(candles[i].t):null;const ds=t?t.toDateString():null;if(ds&&ds!==prevD){ct=0;cv=0;ct2=0;prevD=ds;}const tp=(h[i]+l[i]+c2[i])/3;ct+=tp*v[i];cv+=v[i];ct2+=tp*tp*v[i];const vw=cv?ct/cv:null;if(vw==null)return null;const variance=(ct2/cv)-(vw*vw);const std=Math.sqrt(Math.max(0,variance));return{v:vw,u1:vw+std,u2:vw+2*std,d1:vw-std,d2:vw-2*std};});};
