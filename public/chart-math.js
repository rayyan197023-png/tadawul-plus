...
const vwap=(h,l,c2,v,candles)=>{let ct=0,cv=0,prevD=null;return c2.map((_,i)=>{const t=candles&&candles[i]?new Date(candles[i].t):null;const ds=t?t.toDateString():null;if(ds&&ds!==prevD){ct=0;cv=0;prevD=ds;}const tp=(h[i]+l[i]+c2[i])/3;ct+=tp*v[i];cv+=v[i];return cv?ct/cv:null;});};
